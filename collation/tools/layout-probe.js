...
const { loadConfig, INPUT_DATA, pagePdfPath, inputBookOf } = require('../src/io');
...
const flatDir = path.join(INPUT_DATA, bookDir, editions[work.shanben].pdfDir);
const totalPages = fs.existsSync(flatDir)
  ? fs.readdirSync(flatDir).filter(f => /\.pdf$/i.test(f)).length
  : require('../src/io').listVolumePages(bookDir, editions[work.shanben].pdfDir.replace(/ocr$/, 'pdf')).length;
...
(async () => {
  ...
  for (const pg of sample) {
    const pdf = pagePdfPath(inputBookOf(workId), editions[work.shanben].pdfDir, pg);
    try {
