...
作品以 `meta.yaml` 的 `stage` 字段定级：**缺省为公开**；`stage: draft` 表示未完成点校，构建与展示按草稿处理。策略：精校优先、宁缺毋滥——经仔细点校并确认版权后方升公开，初稿仅小范围交流。

草稿两种展示模式：

| 模式 | 触发 | 行为 | 例 |
|---|---|---|---|
| 截断（默认） | `stage: draft` | 数据含全帙正文时，构建期 payload 截断为**首叶书影**（防源码泄露），不出 PDF/图像 | lunyu6–8 |
| 导览 teaser | `draftNotice.display: teaser` | 数据本身即安全内容，全文渲染：**序全文 + 每章「章名 → AI 导语（含推荐词）→ 核心摘句（大字原文）+ AI 解读（小字）」** | 长物志六帙、遵生八笺八帙 |

- 草稿页无下载按钮，卷末附「需點校」提示卡：正文未公开声明 + 外部公开站全文链接 + 点校群招募；首页列表仍列其名并标【需點校】
- 版权边界：古籍原文公有领域，每章仅最小引用 3–5 句；标点自施，不抄现代点校版；导语/解读为 AI 原创
- 升公开：点校藏事、双人复核后移除 `stage` 字段重建即可；teaser 数据中的导览块可保留为「篇前小序」或删除
