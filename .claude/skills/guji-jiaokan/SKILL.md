---
name: guji-jiaokan
description: AI 古籍校勘通用流程——以公开善本为底、现代点校本对校，经视觉互证+版面结构判定+校书官智能体，产出善本点校本（公开）与现代本（自用）。核心方法论「版面结构先行」：先抽样总结该书版面网格，由顶格/退格规则客观推导经注，而非让视觉主观猜。用于把任意古籍扫描+OCR 加工成可进兰木引擎（songke-facsimile 影刻直出 / songke 宋版善刻）的点校数据。新古籍（中庸/论语/孟子等）照本技能逐步执行即可。
---
