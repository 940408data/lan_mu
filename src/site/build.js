...
function collectSiteChars(site) {
  ...
  for (const t of TOPICS) { add(t.title); add(t.desc); (t.virtual || []).forEach(add); }
  add(COPY.soon.title); add(COPY.soon.sub); add(COPY.back); add(COPY.topicLabel); add(COPY.topicHead);
