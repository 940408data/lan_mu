...
### 站点页（src/site/）：首页「藏书」+ 三藏页 + 书库 + 书目页「目录叶」

作品页之外的站点页，视觉与宋刻同源（暗案底/纸墨/朱记）：

- **首页** `dist/index.html` — 只列「书」不列卷：顶部检索框（书名/卷次/篇名即输即显，繁简双轨——简体串构建期 opencc 预转入索引）+ 文质页签（「四時幽賞」四时手卷**卷影屏** /「四書涵泳」瓷青书影，无 JS 时两面板直陈）+ 专题推荐卡 + 底端「三藏」入口。书影为瓷青封面线装（左上签条题名、左缘订线、右下朱印）立于座上；多卷书 → 目录页，单卷（手卷等无 `book` 块者）→ 直达作品页。
- **三藏页** `dist/sanzang/index.html` — 0327 门户首页迁存（annotated tag「三藏首页」）：检索 + 儒释道部类页签（佛家/道家为虚拟典籍，点入「敬请期待」；无 JS 仅见首部）+ 专题推荐。深层藏，仅由首页底端链接进入，不主动推。
