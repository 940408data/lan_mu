...
### 站点页（src/site/）：首页「藏书」+ 经史子集页 + 书库 + 书目页「目录叶」

作品页之外的站点页，视觉与宋刻同源（暗案底/纸墨/朱记）：

- **首页** `dist/index.html` — 只列「书」不列卷：顶部检索框（书名/卷次/篇名即输即显，繁简双轨——简体串构建期 opencc 预转入索引）+ 文质页签（「四時幽賞」四时手卷**卷影屏** /「四書涵泳」瓷青书影，无 JS 时两面板直陈）+ 专题推荐卡 + 底端「古典·经史子集选编」入口。书影为瓷青封面线装（左上签条题名、左缘订线、右下朱印）立于座上；多卷书 → 目录页，单卷（手卷等无 `book` 块者）→ 直达作品页。
- **经史子集页** `dist/sanzang/index.html` — 0327 门户首页迁存（annotated tag「三藏首页」）：检索 + 经史子集部类页签（经选/史选/子选/集选；未收录书为虚拟典籍，点入「敬请期待」；无 JS 仅见首部）+ 专题推荐。深层藏，仅由首页底端链接进入，不主动推。
- **书库** `dist/shuku/index.html` — 真书全帙一览，按 `meta.category` 分部（經/子/書/禮樂），部类速达锚点。
- **卷影**：`src/site/panels.js` 构建期从四时手卷整卷 PNG 的右起卷首文字区裁 560px 竖条 → `dist/assets/topics/<id>.jpg`（近黑阈值 + 连续 run 检测定位文字区，裱边/绢纹/兰叶/朱印不干扰）；缺图时渲染侧落瓷青渐变占位，dev 免构建可预览。
...
- **双轨生成**：`tools/serve.js` 对 `/`、`/sanzang/`、`/shuku/`、`/books/<id>/` 等站点页动态渲染（dev 免重建）；`tools/gen-index.js` 与全量 build 末尾产出静态文件（生产）。
